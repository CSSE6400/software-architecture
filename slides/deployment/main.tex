...
\begin{frame}{GitLab Flow \cite{github-flow} \cite{gitlab-flow}}
    \vspace{1pt}
    \begin{columns}
...
\begin{frame}{Release Branches \cite{branch-strategies}}
    \vspace{1pt}
    \begin{columns}
    \column{0.6\textwidth}
      \vspace{-5mm}
...
\begin{frame}{Canary Deployment}
    ...
    \end{columns}
\end{frame}
\note{Commonly used to see if something works or will fail in production}
...
\begin{frame}{A/B Deployment}
    ...
    \end{columns}
\end{frame}
\note{Requires sophisticated infrastructure and analytics to do well}
...
\note[itemize]{
    ...
    \item All traffic is sent to \textbf{V1} \highlight{and} \textbf{V2}
    ...
    \begin{itemize}
        ...
        \item When customer checks out their shopping cart,
              you don't want to send \highlight{two} payment requests from \textbf{V1} \highlight{and} \textbf{V2}
        \item \highlight{Mock} external services    
    \end{itemize}
...
\end{frame}
\note{No perfect deployment strategy\\ ~~~-- There are always tradeoffs}
...
\end{frame}
\note{
    Modification of original diagram at\\ https://thenewstack.io/deployment-strategies/
    \begin{itemize}
