% Provide an extended Terraform example.
% Show/explain terraform init/plan/apply/output/destroy.
...
\note[itemize]{
    \item Things got complicated quickly
    \begin{itemize}
        \Large
        \item We need more hardware and it is easier to provision
    \end{itemize}
    \vspace{0.5em}
    \item Largely thanks to virtualisation
    \begin{itemize}
        \Large
        \item No physical activity to spin up a new machine
    \end{itemize}
}
...
\end{code}
\end{frame}
\note{Using AWS CLI to create EC2 access like in the practical}
...
\note[itemize]{
    \item \highlight{Snowflakes} magical machines that `just work' and everyone is afraid to touch
...
\end{code}
\end{frame}
\note{Compliance testing example}
...
\point[\Large Week 4 Prac]{Learn how to use Terraform to write IaC and deploy resources on AWS.}
