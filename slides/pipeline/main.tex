...
\question{Can you name a \highlight{pipeline archiecture}?}
...
\begin{tikzpicture}[>=latex']
    ...
    \node [block,label=\only<3->{\only<3>{\color{focus}}Tester}, right =1cm of input] (grep) {grep "f***"};
    \node [block,label=\only<4->{\only<4>{\color{focus}}Transformer}, right =1cm of grep] (wc) {wc -l};
    \node [block,label=\only<5->{\only<5>{\color{focus}}Consumer}, right =1cm of wc] (tee) {tee anger.txt};
...
\question{What's the advantage of the map reduce pattern?}
