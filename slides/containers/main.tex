...
\note[itemize]{
    ...
    \item Second chroot -- Exists but it can't find files on which \texttt{ls} depends.
}
...
\end{frame}
\note[itemize]{
    \item \texttt{ldd} -- Lists dependencies
    \item \texttt{cp} -- Copy dependencies
...
\end{frame}
\note[itemize]{
    \item Can now run \texttt{chroot . /bin/ls} -- Everything is available from new root
