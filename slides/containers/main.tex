...
\command{docker exec [container]}{
    Run a command in a running \highlight{container}.
}{
    \item[-d] Run the container in the background
\item[-p] Publish a container's port to the host
\item[-v] Mount a volume
...
\item[-l] Remove the specified link
...
\item[-f] Filter output based on conditions provided
\item[-q] Only display numeric IDs
}

\command{docker pull [image]}{
    Pull an image or a repository from a registry.
}{
    \item[-a] Download all tagged images in the repository
}

\command{docker push [image]}{
    Push an image or a repository to a registry.
}{
    \item[-a] Upload all tagged images in the repository
}
...
\point{This turned out to be very common\dots
...
\item[-f] Specify an alternate compose file (default: docker-compose.yml)
}
