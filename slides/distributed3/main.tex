...
\questionanswer{How do we detect faults?}{
    \begin{itemize}[<+(1)->]
        \item No listener on port -- RST or FIN packet
        		\note<2>{Assumes node is running \& reachable\\ \vspace{0.3em}
        		         OS should close or refuse connection\\
        		         \large ~~~~~-- Resulting in a RST or FIN packet being sent\\ \vspace{0.3em}
        				 \Large Error packet may be lost in transit}
        \item Process crashes -- Monitor report failure
        		\note<3>{Assumes node is running \& reachable\\ \vspace{0.3em}
        		         Most reliable}
...
\questionanswer{Can we design a system to be Byzantine fault tolerant?}{
    Yes, but, it is \highlight{challenging}.
}
\note[itemize]{
...
\note{
    Still issues of
    \begin{itemize}
        \item pending messages getting older and
        \item transient network faults exceeding timeout
    \end{itemize}
}
...
\begin{frame}{Handling Poison Messages}
    ...
    {\LARGE
        ...
        \begin{itemize}
            ...
            \Large\item Suitable when message processing \highlight{speed} is most important
        \end{itemize}
        \pause
        \vspace{3mm}
        Always Retry
        \begin{itemize}
            ...
            \end{itemize}
            \Large\item Suitable when message \highlight{delivery} is most important
            \Large\item Potential \highlight{very long delays} in processing
        \end{itemize}
    }
\end{frame}
...
\note{Emphasise that this is \highlight{\textbf{different}} to a poison message}
...
\note[itemize]{
    ...
    \item It must be the last message received by \highlight{all} consumers
}
...
\point[\huge Eventual Consistency]{
\vspace{4mm}
\begin{itemize}
...
\point[\huge Linearisability]{
\vspace{3mm}
\begin{itemize}
    \item<1-> Once value is written, all reads see \highlight{same} value
    \begin{itemize}
        \LARGE\item<1-> Regardless of replica read from
        	\note<1>{Abstraction over replicated database\\ \vspace{0.3em} Used when \highlight{uniqueness} needs to be guaranteed}
    \end{itemize}
    \vspace{2mm}
    ...
    \begin{itemize}
        ...
        \LARGE\item<2-> Read from synchronous follower
        	\note<2>{SLR -- defeats most performance benefits\\ \vspace{0.3em} Maintains reliability benefits}
    \end{itemize}
    \item<3-> Multi-leader replication can't be linearised
    \vspace{2mm}
    \item<4-> Leaderless replication
    \begin{itemize}
        \LARGE\item<4-> Lock value on quorum \highlight{before} writing
        	\note<4>{Leaderless -- similar performance cost to SLR}
    \end{itemize}
\end{itemize}
}

\point[\huge Causal Order]{
\vspace{4mm}
\begin{itemize}
    \item<1-> Order is based on causality
    \begin{itemize}
        \LARGE\item<1-> What event needs to happen \highlight{before} another
...
\point[\huge Consensus Properties]{
\vspace{3mm}
\begin{description}
...
\point[\huge Two-Phase Commit]{
\vspace{4mm}
\begin{description}
    \item[Prepare] Confirm nodes can commit transaction
    \vspace{2mm}
    \item[Commit] Finalise once consensus is reached
    \begin{itemize}
...
\note[itemize]{
    \item Transaction ID used to track writes \\~~~ -- \texttt{transId} returned by \texttt{start()}
    ...
    \begin{itemize}
        ...
        \end{itemize}
      	\note<1>{
      	    ...
      	    \begin{itemize}
      	        \item network delays,
      	        \item process pauses, and
      	        \item clock error
      	    \end{itemize}
      	    i.e. Never exceed an upper bound\\
      	    \vspace{0.5em}
      	    Not realistic due to unbounded delays as discussed previously
      	}
        \vspace{2mm}
        ...
        \end{itemize}
      	\note<2>{
      	    Assumes it behaves synchronously \highlight{most of the time}\\
      	    \vspace{0.4em}
      	    Realistic for many systems\\
      	    ~~~ -- Networks and processes \highlight{usually} are well behaved\\
      	    \vspace{0.4em}
      	    Design error handling to \highlight{manage} arbitrarily large delays
      	}
        \vspace{2mm}
        ...
        \begin{itemize}
            ...
            \Large\item Difficult \& restrictive design options
        \end{itemize}
      	\note<3>{
      	    Assumes delays will occur and will affect system logic\\
      	    \vspace{0.4em}
      	    Design system to manage all ordering / timing issues\\
      	    ~~~ -- Do not use a clock for ordering\\
      	    \vspace{0.4em}
      	    Managing timeouts is \highlight{difficult}\\
      	    ~~~ -- Expects \highlight{arbitrarily} large delays\\
      	}
    \end{itemize}
    }
\end{frame}
...
\begin{frame}{Distributed Systems Node Failure Assumptions}
    ...
    \begin{itemize}
        ...
        \end{itemize}
      	\note<1>{e.g. Cloud-based system that kills crashed nodes}
        \vspace{4mm}
