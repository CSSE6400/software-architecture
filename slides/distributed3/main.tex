...
\note[itemize]{
    \item Sequence of slides with an animation of a poison message.
    \item First 3 slides are an example of a message being queued and processed.
    \item Slides 4-8 are an example of a poison message blocking the queue.
    \item Should comment that poison messages block processing regardless of how they're delivered.
...
\begin{frame}
    \begin{columns}[onlytextwidth,c]
      ...
      \column{0.5\textwidth}
        \includegraphics[width=0.5\textwidth]{images/poison-bottle.png}  % From https://pixabay.com/vectors/poison-bottle-toxic-chemistry-4662212/
    \end{columns}
\end{frame}
\note[itemize]{
...
\begin{itemize}
    ...
    \begin{itemize}
        \LARGE\item e.g. Social media updates\footnote{See \href{https://csse6400.uqcloud.net/slides/distributed2.pdf}{Distributed II slides 40 - 44}.}
    \end{itemize}
\end{itemize}
}
...
\begin{frame}{Distributed Systems Node Failure Assumptions}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \large\item Requires persistent memory to recover to close to prior state
            \end{itemize}
        \end{itemize}
        \vspace{2mm}
...
\end{frame}
\note[itemize]{
    \item Crash Stop -- Cloud-based system that kills crashed nodes.
    \item Crash Recovery -- Any system that allows nodes to be restarted.
    \item Crash Recovery -- May lose some steps in memory for non-critical tasks.
    \item Arbitrary Failure -- Nodes may send faulty or malicious messages.
}
