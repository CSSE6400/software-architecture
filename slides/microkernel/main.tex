...
\questionanswer{Does a plug-in architecture equate to a microkernel architecture?}
{What about the \highlight{Linux kernel}?}
\note[itemize]{
    \item Not a microkernel -- kernel has a lot of complex logic
    \vspace{0.5em}
    \item Core of a microkernel provides a framework in which plug-ins deliver functionality
}
...
\begin{frame}{Distributed Microkernel}
    \begin{itemize}
        ...
        \begin{itemize}
            \large
            \item Technical or domain partitions
            \item Plug-ins could also be distributed
        \end{itemize}
    \end{itemize}
    \vspace{3mm}
    \centering
...
\begin{frame}{Media Server \& Renderer}
    \begin{adjustwidth}{-10mm}{-10mm}
        \centering
        \includegraphics[trim=99 140 99 20,clip,width=0.98\paperwidth]{diagrams/overall_context}
    \end{adjustwidth}
\end{frame}
...
\begin{frame}{Domain Colour Key}
    \begin{adjustwidth}{-7.5mm}{-7.5mm}
        \includegraphics[trim=20 595 22 96,clip,width=0.97\paperwidth]{diagrams/key.png}
    \end{adjustwidth}
\end{frame}
\note[itemize]{
    ...
    \item Plug icons for Components (shown on later slides)
}


\begin{frame}{Media Renderer}
    \vspace{3mm}
    \includegraphics[trim=80 100 80 20,clip,width=\textwidth]{diagrams/renderer_container}
\end{frame}
\note[itemize]{
...
\begin{frame}
    ...
    \begin{adjustwidth}{-10mm}{-10mm}
        \centering
        \includegraphics[trim=135 128 125 50,clip,height=0.95\paperheight]{diagrams/media_server_container}
    \end{adjustwidth}
	\begin{tikzpicture}[overlay,remember picture]
	\node[rectangle,shift={(3.65,3.85)},
		  text width=6.8cm,above] at (current page.west) {\color{primary}\Large Media Server};
	\end{tikzpicture}   
\end{frame}
\note[itemize]{
    \item Renderer uses server to allow consumer to browse and watch content on the server
    \vspace{1em}
    \item Core Media Server simply coordinates how the plug-ins deliver behaviour
    \begin{itemize}
        \Large
        \item \highlight{Simplicity} quality attribute
        \item Example of why it is a \highlight{micro}kernel
    \end{itemize}
    \vspace{1em}
    \item Four types of plug-ins deliver customised behaviour
    \begin{itemize}
        \Large
        \item Each corresponding to a media server \highlight{domain}
    \end{itemize}
}
...
\begin{frame}
    ...
    \begin{adjustwidth}{-10mm}{-10mm}
        \centering
        \includegraphics[trim=54 100 54 0,clip,height=0.97\paperheight]{diagrams/media_server_core_component}
    \end{adjustwidth}
	\begin{tikzpicture}[overlay,remember picture]
	\node[rectangle,shift={(3.65,3.85)},
		  text width=6.8cm,above] at (current page.west) {\color{primary}\Large Core Media Server Components};
	\end{tikzpicture}   
\end{frame}
\note[itemize]{
...
\begin{frame}{Library Management Plug-ins Components}
    \vspace{4mm}
    \centering
    \includegraphics[trim=165 160 165 195,clip,width=0.95\textwidth]{diagrams/libmgt_component}
\end{frame}
\note[itemize]{
    \item Components scrape metadata from different sources
    \item \highlight{Scrape Metadata} is the interface implemented by the plug-ins
}
...
\note[itemize]{
    ...
    \vspace{0.5em}
    \item MediaData \& MetaData classes pass standardised data structures to and from each scraper
    \begin{itemize}
        \Large
...
\begin{frame}
    ...
    \begin{adjustwidth}{-10mm}{-10mm}
        \centering
        \includegraphics[trim=227 250 218 290,clip,width=1.07\textwidth]{diagrams/deployment}
    \end{adjustwidth}
	\begin{tikzpicture}[overlay,remember picture]
	\node[rectangle,shift={(3.65,3.8)},
		  text width=6.8cm,above] at (current page.west) {\color{primary}\Large System Deployment};
	\end{tikzpicture}   
\end{frame}
\note[itemize]{
    ...
    \begin{itemize}
        \Large
        \item Plug-ins use the metadata stored in the Library Database
        \item Plug-ins use the media files stored on the File System
    \end{itemize}
}
