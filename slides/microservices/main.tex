...
\note[itemize]{
    ...
    \vspace{0.5em}
    \item Often introduces \highlight{synchronous} flow of logic between services
    \begin{enumerate}
        \item \large \highlight{Customer} places an \highlight{Order}
        \item \large \highlight{Order} queries \highlight{Product} to get details
        \item \large \highlight{Product} queries \highlight{Pricing} to determine price
    \end{enumerate}
}
...
{
    Minimise coupling between services.
}
...
\point[\Large Restaurant Examples]{
    ...
    \note<1>{
        \begin{itemize}
            ...
            \begin{itemize}
                \large
                \item Drinks, Entrée, Main, Dessert, ...
            \end{itemize}
            \item ...
        	\end{itemize}
    }
...
\note{
    In 2017, Uber had over \highlight{1400} services
	\begin{itemize}
	    \Large\item Consider how bad coupling would be with either approach
	\end{itemize}
}
...
\end{frame}
\note[itemize]{
    \item Use the tried and true \highlight{Observer} pattern
	\begin{itemize}
	    \Large\item With the \highlight{Event-Driven Architecture} pattern
	\end{itemize}
    \vspace{0.2em}
...
\note[itemize]{
    \item Probably different \highlight{business processes}
	\begin{itemize}
	    \Large\item But possibly the same \highlight{context}
	\end{itemize}
	\vspace{0.6em}
    \item If separate services:
	\begin{itemize}
	    \Large
	    \item Browse needs to send an event for every change to the shopping cart
		\item Purchase needs to listen for these
	\end{itemize}
	\vspace{0.3em}
...
\point[\Large Model Behaviour]{
    \begin{itemize}
        ...
        \vspace{0.5em}
        \item Help you to better model your \highlight{domain}
        \vspace{0.5em}
