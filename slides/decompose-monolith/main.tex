        ...
        \note<3>{Migration
				 \begin{itemize}
        			 ...
        			 \item Some limitation on choice of architecture\\
        			       ~~-- Most sophisticated architectures can be used\\
        			       ~~~~~~-- Requires more planning, design, \& work
        		 \end{itemize}
        		}
    \end{itemize}
}
    ...
    \column{0.65\textwidth}
      {\LARGE
        \begin{itemize}[<+(0)->]
            ...
            \note<1>{May already have an API if the UI is a web or mobile app}
            \vspace{2mm}
            \item Proxy intercepts API calls
            \begin{itemize}[<2->]
                \Large\item Proxy directs calls to application or new services
            \end{itemize}
            \note<2>{Initially, deploy proxy and new interface into production\\
                     ~~-- With only existing monolith\\
                     \vspace{0.5em}
...
{\huge
\begin{itemize}[<+->]
    \item Identify \highlight{bounded-contexts}
    \vspace{3mm}
    \item \highlight{Simple} first service
    \begin{itemize}[<2->]
        \LARGE\item e.g. Authentication
    \end{itemize}
    \note<2>{Use first service (or first few) to validate approach and deployment infrastructure}
    \vspace{3mm}
    \item \highlight{Minimise} dependency from services to monolith
    \begin{itemize}[<3->]
    ...
    \note<3>{\begin{itemize}
                 ...
                 \begin{itemize}
                     \Large\item Expect a fair bit of work removing features from monolith
                 \end{itemize}
             \end{itemize}
            }
\end{itemize}
...
{\huge
\begin{itemize}[<+->]
    \item \highlight{Reduce} coupling between bounded-contexts
    \vspace{1mm}
    \begin{itemize}[<1->]
    ...
    \vspace{3mm}
    \item Decouple \highlight{vertically}
    \vspace{1mm}
    \begin{itemize}[<2->]
...
\end{tikzpicture}
    \note<2>{\begin{itemize}
                 ...
                 \begin{itemize}
                     \Large\item Need to implement \highlight{all} parts of the business process
                     \begin{itemize}
...
{\huge
\begin{itemize}[<+->]
    \item Focus on \highlight{pain points}
    \begin{itemize}[<1->]
    ...
    \vspace{3mm}
    \item \highlight{Rewrite}, don't reuse
    \begin{itemize}[<2->]
...
\point[\huge Stepwise Decomposition]{\vspace{0.5em}Replace application functionality one service at a time.}
