...
\point[\Large Distributed Systems Series]{
    ...
    \begin{labeling}{Distributed III}
        ...
        \item[\color{secondary} Distributed II] \color{black}\highlight{Complexities} of \highlight{stateful} systems
...
\note[itemize]{
    \item Database has state, \highlight{persistent} data
...
\vspace{1em}
    \begin{labeling}{Stateless}
        \item[\color{secondary} Stateless] Does \highlight{not} utilise \highlight{persistent data}.\\
             Or: each request is independent. 
        \vspace{1em}
        \item[\color{secondary} Stateful] Does utilise \highlight{persistent data}.\\
             Or: server or service remembers and uses data from previous interactions.
    \end{labeling}
}
...
\note[itemize]{
    ...
    \item Multiple followers, only \highlight{one} leader
}

\definition{Leader-based Replication}{
    One node (the leader) handles all write operations,
    and multiple other nodes (followers) replicate the data and handle read operations.
}
...
\note[itemize]{
    \item Different services could read from different replicas
    \vspace{0.5em}
...
\definition{Stale data}{
   Outdated or inconsistent data that does not reflect the latest updates, mainly due to replication delays,
   caching, or network issues in distributed systems.
}

\image{diagrams/ReplicationLag}
\note{Update of Lifelike Inflatable \highlight{Nicholas Cage} is not propagated to a follower before it's queried}

\definition{Replication Lag}{
    The time taken for replicas to update \highlight{stale} data.
}
\note{\highlight{Replication Lag}: Time it takes for the product name change to update across \highlight{all} followers.}

\image[height=\textheight]{diagrams/AsyncLag}
\note{Purple lifeline bar is \highlight{replication lag}}
...
\note[itemize]{
    ...
    \item \highlight{Replication lag} means reading a field immediately after updating it \highlight{may} lead to reading \highlight{stale} data.
}

\definition{Read-your-writes Consistency}{
    Users always see the updates that \highlight{they have made}, even though others may see stale data.
}
...
\vspace{1em}
\begin{itemize}[<+->]
    \item If you have multiple leaders, you can write to any,
...
\note[enumerate]{
    ...
    \item Customer orders 20 pillows, the number that \highlight{they saw} were available.
    \item -1 Pillows?
...
\begin{labeling}{On Write}
    ...
    \item[] e.g. Last Write Wins (LWW) in DynamoDB.
    \note[item]<1->{Bucardo allows a perl script for on write resolution\vspace{3em}}
    \vspace{0.25em}
    \pause
    \item[\color{secondary} On Read] Store all conflicting versions on write.
         When a conflict is next read, ask for a resolution.
    \item[] e.g. \emph{git pull}
    \note[item]<2->{CouchDB prompts reads to resolve the conflict}
\end{labeling} 
}
...
\note[itemize]{
    ...
    \item Resurgence after Amazon created Dynamo
...
\begin{frame}{Leaderless Replication}
  \begin{adjustwidth}{-10mm}{-10mm}
    \centering
    \includegraphics[width=0.98\paperwidth]{diagrams/Leaderless}
  \end{adjustwidth}    
\end{frame}
\note{Reads and writes can be written to any node}

\point[\Large How does IT work?]{
    \vspace{0.5em}
...
\point[\Large How are changes propagated?]{
    ...
    \begin{itemize}[<+->]
        \item Read Repair
        \note[item]<1->{Read Repair: Client detects stale data on read and writes updated data to that replica
                        \vspace{1em}}
        \vspace{0.5em}
...
\note{Reading from a single replica means we \highlight{can't} know if data is stale or inconsistent}

\questionanswer{How do we know it's consistent?}{Quorum Reads and Writes}
...
\note[item]<1->{Nodes read from must overlap with the nodes written to\vspace{1em}}
...
\note[item]<2->{Regardless of number of reads or writes}
...
\begin{description}
    ...
    \item[$\textcolor{eq1}{w}$] amount of replicas to {\color{eq1}\textit{write}} to
    \item[$\textcolor{eq2}{r}$] amount of replicas to {\color{eq2}\textit{read}} from
\end{description}
}
...
\end{frame}
\note[itemize]{
    \item Graphical representation of previous equation \vspace{1em}
    \item \textcolor{eq2}{\textbf{Orange}} inner group (\highlight{reads}), overlaps with \\ 
          \textcolor{eq1}{\textbf{Blue}} inner group (\highlight{writes}) \vspace{1em}
...
\note{
  ...
  \begin{enumerate}
    ...
    \item -1 Pillows?
    \item How do we resolve this?
    \item Potentially manual process -- contact customer and inform them of the stock deficiency
  \end{enumerate}
}
...
\note[itemize]{
    ...
    \item Allows scalability of \highlight{large data}, not just \highlight{large load}
...
\note{e.g. Partitioning based on primary key, student ID}
...
\note{Hash tables hash entries to maximise the spread between buckets}
...
\note[itemize]{
    \item Mention microservices should use independent databases
    \vspace{0.5em}
    \item Other ways to design independent databases are out of scope for CSSE6400
}
