...
\begin{itemize}[topsep=5pt,partopsep=1pt,itemsep=3pt,parsep=3pt]
  \item Each team will be allocated \textbf{40 minutes} during \textbf{Week 13} to deliver their presentations.
  ...
  \item The teaching team will provide a list of available time slots.
        These will include the scheduled lecture, case study, and practical sessions in week 13,
        but presentations at other times will be required.
  \item Your team must collaboratively choose a \textbf{single time slot} in which \textit{all} team members are available.
...
Start by introducing your team's capstone project.
Give an elevator pitch style summary of what problem the project solves and its key features.
Spend \textbf{less than 1.5 mins} on this introduction.

Describe the project's software architecture using appropriate views \cite{view-notes}.
You must use the C4 modelling notation \cite{view-notes} \cite{brown2026c4} to describe the software architecture.
It is expected that you will get down to the level of component diagrams,
though you should not need to describe the details of every component.
...
You are not expected to describe the design trade-offs,
which will be done by \hyperref[sec:Critique]{\textbf{Presenter \#2}};
nor get down to the level of describing the detailed design of the software,
which will be done by \hyperref[sec:DetailedDesign]{\textbf{Presenter \#3}}.
...
\subsubsection{Presenter \#2 (5 minutes): Critique}\label{sec:Critique}
...
Begin by describing which ASRs and, in particular,
the quality attributes you think are most important for the project and why.
...
\subsubsection{Presenter \#3 (5 minutes): Detailed Design}\label{sec:DetailedDesign}

Your presentation should focus on the \textbf{detailed design} of \textbf{one or two key parts} of your system.
...
You need to show how key components work together to fulfil the system's ASRs and quality attributes.

Select only \textbf{one or two} significant components, services, or subsystems.
What you select should be central to the system's functionality. Describe their internal structure, key interfaces,
and important interactions with other parts of the system.

You should use appropriate C4 and UML diagrams to support your explanation.
Depending on what you are describing, these may include:

\begin{itemize}[topsep=5pt,partopsep=1pt,itemsep=3pt,parsep=3pt]
    \item \emph{Component diagrams} to introduce communication pathways.
    \item \emph{Dynamic diagrams} to show interactions between components.
    \item \emph{Deployment diagrams} to show details of the distributed design.
    \item \emph{Class diagrams} to show internal structure and relationships.
    \item \emph{Sequence diagrams} to show interactions between components or classes.
...
For example, if you have used an adapter, explain why and how it contributes to quality attributes.

You do not have time to describe the full system in detail.
...
Summarise the key points from the entire presentation in the conclusion.
Highlight lessons learnt from designing the system.
Be clear and concise, taking about \textbf{30 seconds}.

\subsubsection{Number of Presenters}

Teams with:
\begin{itemize}[topsep=5pt,partopsep=1pt,itemsep=3pt,parsep=3pt]
    \item \textbf{Five} members, only need to have \textbf{one} comparison to an alternative architecture.
    \item \textbf{Seven} members, need to have \textbf{three} comparisons to an alternative architecture.
\end{itemize}
...
You must make your presentation \textbf{in-person}.
...
Your slides are due at \textbf{15:00 on 25 May 2026}.
...
\link{course profile}{https://course-profiles.uq.edu.au/course-profiles/CSSE6400-21413-7620}.
