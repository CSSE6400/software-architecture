...
Inside the \texttt{tests} folder create a \texttt{base.py} file.
Add the following code to \texttt{base.py}.
...
Open the \texttt{todo/\_\_init\_\_.py} file and update the \texttt{create\_app} function to be as follows.
...
\hint{
  To retrieve the value of a request parameter, use 
...
\begin{itemize}
  ...
  \item filter todo items by whether they are completed or not,
        using the \texttt{completed} query parameter in the list get query); and
  \item filter todo items by whether they are within the next \textit{N} days,
        using the \texttt{window} query parameter.
\end{itemize}
