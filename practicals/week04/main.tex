...
\author{Brae Webb \& Richard Thomas \& Millie Hughes}
...
You will use the lab to learn how to deploy and manage infrastructure with AWS.
...
it is a general purpose image.
Every AMI has a unique AMI code, which changes every time the image is updated.
A recent AMI for the Amazon Linux 2023 is \texttt{ami-0c421724a94bba6d6}.
%Search for it via the code and select it.

%NOTE: If you change the AMI, you need to update all references below.
%NOTE: The tests/test_terraform.py unit tests also needs the AMI changed in the EXPECTED_INSTANCE.
...
\begin{center}
  ...
  \begin{adjustwidth}{-8mm}{-8mm}
    \includegraphics[width=0.96\paperwidth]{images/launch-instance}
  \end{adjustwidth}
\end{center}
...
\item Select an appropriate AMI (e.g. Amazon Linux kernel-6.18 2023 AMI, \texttt{ami-0c421724a94bba6d6}). You may need to select the drop down list to see it, or search for it by its AMI.
...
\item For Key pair (login), select the existing \texttt{vockey} (Type: rsa) key pair option.
...
\item In the root directory of your repository,
      create a \texttt{credentials} file and copy the contents of the text block into the file.
      \textbf{Do \underline{not} share this file contents --- do \underline{not} commit it}.
      This file is already in the \texttt{.gitignore} of your practical repository.
...
The \texttt{terraform} block specifies the required external dependencies, here we need to use the AWS provider above version 6.0.
...
First we need a script to provision the server, if we combine all our commands from section \ref{sec:installHextris}, we will produce the following script.
...
We do not want to go hunting through this file all the time.
Instead we will use the \texttt{output} keyword.

We can specify certain attributes as `output' attributes.
Output attributes are printed to the terminal when the module is invoked directly but,
as we will see later, they can also be used by other Terraform configuration files.
...
The output can be formatted using Terraform's string interpolation.
We can use this to output the Hextris server's URL, rather than just its IP address.
...
We can use the \texttt{terraform output} command to return the value of output variables.
We can specify which variable(s) to return and the format in which they are returned.
We can use this to obtain the Hextris server URL in a format that can be pasted directly into a browser.
This can be run anytime after applying the plan.
...
Here we have specified to return the value of the \texttt{hextris-url} variable, without any formatting.
...
Refer to the Terraform documentation for details or, as is often quicker,
\href{https://www.google.com/search?q=terraform+aws+security+group}{Google ``terraform aws security group''}.
...
\footnote{Ingress and egress in networking just mean incoming and outgoing respectively.}
...
In the \texttt{aws\_instance.hextrix-server} resource, include the following line.
...
If you paste the URL into a web browser (the IP address may have changed),
...
\notice{
  Destroy everything again before you finish and close your Learner Lab.
  Anything left running in your lab may continue to consume your credit.
}
...
From within your repository, the following script will run the tests locally.
...
Since CSSE6400 runs this practical every year, sometimes the AMI that we are using is out of date or does not exist any more.
...
If we run \texttt{terraform plan}, we may see that it wants to destroy and recreate the EC2 instance.
This would be because the AMI has changed since this practical was last updated.
