...
This week, \texttt{Path A} offers an alternative method to autoscaling.

\subsection{[Path A] EC2}

Congratulations! You have chosen to go down the EC2 path.
This week we need to create an AutoScaling Group and a Load Balancer to handle the scaling of our service.
...
Instead of manually deploying an instance we are going to tell AWS how to deploy instances for us.
...
Some of these changes have been highlighted below.
...
The AutoScaling group is given the instance template and uses it to create groups of instances.
The group defines the desired capacity and the limits of how many instances we want.
...
If we deployed this now, we would get one initial instance created for us,
based on the \texttt{desired\_capacity} and \texttt{min\_size} values.
...
This is the side that lets traffic from the load balancer hit the instances.
...
We also need to link the \texttt{target\_group} to the autoscaling group.
...
We now need to create a load balancer and route traffic to our target group.
...
The load balancer now needs a way to accept incoming traffic.
We need a listener which creates the rules for how traffic is routed to the instances.
...
To make our autoscaling group perform dynamic scaling,
we need to create some rules for when to scale up and when to scale down.
The policies below track the average CPU usage of the group of instances.
...
We also apply a cooldown period so that we do not act too aggressivly to changes in CPU usage,
