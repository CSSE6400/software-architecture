...
\author{Millie Hughes, Brae Webb \& Richard Thomas}
...
\info{
GitHub Codespaces has Docker with the \texttt{docker compose} command installed.
}
}  % end \notice

\teacher{
  Tell students who have not yet installed Docker to do so now, while you are working through the following material.
}


\section{Getting Started with Docker}

In the lectures we delved into the world of containers and how they are used to create a consistent environment for our applications.
...
\notice{
  This practical will assume you have not used Docker before.
  We will skip over some aspects that are not needed for our purposes.
  Please watch the lecture \cite{container-slides} and read the notes \cite{container-notes}
  for a foundational understanding of container technology and terminology.
}
...
They are self contained environments that should contain the dependencies and code for any application.
A common way to build an image is to use a \texttt{Dockerfile}.

\subsubsection{Dockerfile}

A \texttt{Dockerfile} is a plain text file with its own syntax that is used to describe the instructions for creating an image.
The series of instructions are run in order to create each layer of the image.

A reference of all possible \texttt{Dockerfile} instructions can be found here:
...
\paragraph{FROM}
The FROM instruction should be the first instruction in your \texttt{Dockerfile} and specifies the base image for subsequent instructions.
If you want to start with an empty image, use the \texttt{scratch} image.
...
The layer will be used for the next step in the \texttt{Dockerfile}.
...
The example above installs Python dependencies for a fake application.
\info{
...
\paragraph{COPY}
The COPY instruction copies files or directories from \texttt{<src>} on the host machine,
 where the image is being built, to the filesystem of the container at the path \texttt{<dest>}.
...
\paragraph{ADD}
The ADD instruction copies files, directories, or remote file URLs from \texttt{<src>}
to the filesystem of the image at the path \texttt{<dest>}.
...
Though the ADD command is usually not recommended and is often replaced with \texttt{curl} or \texttt{wget}.
...
If the WORKDIR does not exist, it will be created even if it is not used in any subsequent \texttt{Dockerfile} instruction.
...
The following example will override the default \texttt{-{}-help} argument
and run the application with the \texttt{-{}-version} argument.
...
This concludes the \texttt{Dockerfile} instructions required for this practical.
There are more that you may need to explore in the future.
The full list of instructions can be found in the
\link{official documentation}{https://docs.docker.com/engine/reference/builder/}.
...
When the container is built, each of these instructions is run to create a layer of the image.
This means that if you change a line in the \texttt{Dockerfile},
you only need to rebuild the layers that come after the altered line.
This is a huge time saver when developing your \texttt{Dockerfile}.

Let's take a look at the \texttt{Dockerfile} below.
In this \texttt{Dockerfile}, our application has Python dependencies that need to be installed.
...
To see the layers that are created when you build your \texttt{Dockerfile} you can use the following command.
...
When we introduced the \texttt{Dockerfile}, we glossed over the details of from where \texttt{ubuntu:latest} is coming.
...
such as \texttt{latest} and \texttt{25.10}.
...
It is worth noting that if you do not use the \texttt{-{}-rm} flag when using \texttt{docker run},
...
We can modify our original command so that the container is not kept around after it terminates as shown below.
...
The first occurrence of the port is the binding on the host machine,
so from the host we can go to localhost:6400 to access it.
The second occurrence is the port being listened to within the container.
...
Docker networking is much more complicated than just this,
but it is a starting point so that we can start using containers that talk to the outside world.
...
If we want this to live update with the contents of our application on the host,
assuming we run this command from the application's directory, we can do so with:
...
Navigate to the GitHub Classroom link for this practical provided on edstem.
...
(Note that some students encounter issues with \texttt{docker compose} when using Codespaces.)
This repository contains our solution to last week's practical exercise.
...
Inside the practical folder create a new file called \texttt{Dockerfile}.
To start, we will use the following \texttt{Dockerfile}.
\begin{code}[language=docker,numbers=none]{Dockerfile}
...
We can do this by adding the following lines to our \texttt{Dockerfile}.
...
Finally we can run our application by adding the following line to our \texttt{Dockerfile}.
...
\newpage
We should now have a complete \texttt{Dockerfile}, as shown below.
...
Now that we have created our \texttt{Dockerfile}, we can build our Docker image.
...
If you have other containers running they can conflict with this container.
}
...
Docker compose allows us to run multiple containers together,
which can be extremely useful for managing complex projects that require numerous run parameters or manage multiple containers.
...
This week we have made a small change to our application
so that it can receive the SQLAlchemy database URI as an environment variable.
Look at the following code in the \texttt{todo/\_\_init\_\_.py} file.
...
This means that we can use \texttt{docker compose} to run our application and database together.
...
% \teacher{
%   Here we will describe the basics of CI for building our container,
%   we teach this here so that next week we can start using it for the AWS deployment.
% }
...
Over the past three weeks we have built a full todo app API
that is ready to be deployed to a remote production environment.
We started with a Flask API returning static responses and
now we have a dynamic API that is running in a containerised environment with an external database.

We have cheated with our Docker deployment by using the Flask development server.
For a production environment you should use a production webserver such as \texttt{gunicorn} or \texttt{uwsgi}.
These web servers are designed to handle multiple requests at the same time and
are much more efficient than the Flask development server.
We recommend you have a look at how to prepare a Flask application for production,
as it may help you in your future projects if you choose to use Flask.
