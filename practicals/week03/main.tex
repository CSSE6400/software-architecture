...
While the docker website suggests using docker-desktop for Linux, the course recommends using the native docker installation.
}
}
...
This ubuntu is a very minimal image that is used intended to be used as a starting point and is itself an image.
...
The RUN command allows us to execute commands when the container is built.
...
The COPY instruction is one of the simplier way to put resources into the container.
This command allows copying individual files and you can also copy entire directories.
Be aware that when you COPY files they will exist in a layer forever.
The following example will still expose the secrets.txt file even though we removed it in another layer.
...
ADD can behave like COPY but it can also pull in files and tars from remote locations.
...
The WORKDIR command is a friendly readability feature to help you reduce the amount of moving around within the container.
...
This means that if we change our application code we don't need to reinstall the requirements.
...
but once your project becomes larger and you have more dependencies it can become a real time saver,
...
When we introduced the Dockerfile we glossed over the details of where \texttt{ubuntu:latest} is coming from.
This image is comes from a registry.
There are a few different registries that you can use.
The default registry is hosted by Docker,
this is the registry that is used when you don't specify a registry and is available at \url{https://hub.docker.com/}.
...
These tags are used to specify different versions of the image so you can pin your image instead of using the \texttt{latest} tag which will always point to the latest version of the image.
...
Lets now change our working directory to \texttt{/app} and copy our \texttt{Pipfile} and \texttt{Pipfile.lock} into the container.
...
You may have noticed that when we defined the todo application service above we didn't specify a container.
This is because Docker Compose can be instructed to use our Dockerfile to build at time so that we dont have to keep using the build command.
...
We started with a Flask API returning static responses and now we have a dynamic API that is running in a containerised environment with a external database.
