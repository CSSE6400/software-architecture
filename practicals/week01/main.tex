...
\author{Brae Webb \& Millie Hughes \& Richard Thomas}
...
% \todo{
%   todo: Add GraphQL and RPC/GRPC content
%   todo: Add SOAP content
% }
...
\section{This Week}
These practicals will set out some fundmental goals for you to accomplish each week, the goals for this week are to:
\task{
\begin{itemize}
  %\item explore techniques used by developers to communicate between components in distributed systems;
...
This semester we will create a scalable and fault-tolerant \textsl{todo list} application.
You need to keep up with the practicals or you will \textbf{\textit{not}} be able to complete the projects.
...
Additionally, committing your work regularly is a good habit to get into and will be useful for your future career.
...
Once you have created an account, you will need to join the course organisation.
If you have not yet filled out the \link{GitHub username registration form}{https://forms.gle/3mZTPKQAL8U7Revu6},%{https://tiny.cc/csse6400reg},
...
You can clone this repository to your local machine or work directly in the browser with GitHub Codespaces.

\teacher{
  If you have trouble with students setting up their environments locally then point them to the GitHub Codespaces option.
  With the amount of students in your class you will probably not have enough time to troubleshoot everything week 1.
}
...
This endpoint should create a new task in the todo list.
The title field must be included in the request and all other values are optional.
The \texttt{created\_at}, \texttt{updated\_at} cannot be set by this method.
...
Feel free to implement the practicals and your assignments in any language with which you feel comfortable,
but be aware that help with other languages or frameworks may be limited.
...
\teacher{
  Let the students setup their initial python environment.
  It may have been some time since these students used Python, so they might have to do a full install including installing pip.
  Some common issues that come up are:
  \begin{itemize}
...
\begin{code}{pyproject.toml}
  ...
  name = "practical01"
  version = "0.1.0"
  description = "Practical guide to implementing a web application programming interface (API) using Flask."
  license =  "CC-BY-NC-SA-4.0"
  authors = [
      {name = "Richard Thomas",email = "richard.thomas@uq.edu.au"}
  ]
  readme = "README.md"
...
\info{
  Your Python version may be different, this is fine as long as it is Python 3.
  For our automated tests we would like you to change the version number in \texttt{pyproject.toml}
  so that it is \texttt{requires-python = ">=3.12"}.
  If you are running a version of Python lower than this, please consider upgrading.
}

\teacher{
  Try to get the students to do the above otherwise you will be answering questions about it towards the end when the tests don't run.
}

Next we are going to add Flask as a dependency to our project.
This library is a small web server wrapper that will allow us to quickly build our todo application.
To add a dependency to our project, we can run the following command.
...
.2 .csse6400.
.3 bin.
.4 clean\_repository.sh.
.4 health.sh.
.4 unittest.sh.
.4 validate\_structure.sh.
.3 tests.
.4 test\_health.py.
.4 test\_todo.py.
.2 .github.
...
This web server is a bit boring though, let's add an endpoint so we can see that it works.
In the \texttt{todo} folder, create a folder called \texttt{views} and a file \texttt{routes.py}.
Add the following code to \texttt{routes.py}.
...
The above has made an endpoint within our API under the \texttt{/api/v1} prefix and we have created a \texttt{/health} route below this.
Now we need to register this with our flask app.
In the \texttt{\_\_init\_\_.py} file, change the contents to the following.
...
\teacher{
  If this is in the Github Codespaces environment then you will have to go to the ports tab and click on the port on which the flask server is running. If this port is set to private then you won't be able to see it, set the port to public and then click on the link to open it in a new tab.
}
...
We have a web server running now, but we are communicating with text instead of a structured format.
JSON is a common format to communicate data between services and is human readable.
To start using JSON we are going to make a small change to our health endpoint.
In the \texttt{routes.py} file, add a new import for jsonify and change the health endpoint to the following.
...
\info{
  For GET requests we can use our browser to call the API,
  but for POST, PUT, and DELETE requests we will need to use a tool like curl or Postman.
}
...
Install curl if it is available for your operating system.
If you are on a Mac, you can install it with homebrew otherwise it is available for most Linux distributions.

Now that you have the tool installed run your API in a terminal window.
Open a new terminal so you can make requests to your API.
Enter the following into your terminal to call your API.
...
If you are using Visual Studio Code for your text editor you can install the ``Rest Client'' by Huachao Mao.
This extension allows you to have files with requests that you can then run from  within the editor.
The benefit of this method is that we can also check this into our repository.

Find the extension in the VS Code marketplace and install it.
Once installed, create a new file in the root of your project called \texttt{endpoints.http} and add the following to it.
...
A subtle ``Send Request'' should be visible between the comment and the GET.
Click on this and a new tab should open up with the response from the API.
The response should be similar to below.

\begin{code}[language=json,numbers=none]{}
  HTTP/1.1 200 OK
  Server: Werkzeug/2.2.3 Python/3.10.9
  Date: Fri, 20 Feb 2026 04:13:00 GMT
  Content-Type: application/json
  Content-Length: 16
  Connection: close
...
\begin{code}[language=bash,numbers=none]{}
  127.0.0.1 - - [20/Feb/2026 14:13:00] "GET /api/health HTTP/1.1" 200 -
\end{code}

\subsubsection{Creating more endpoints}

Let's expand our \texttt{endpoints.http} file to include the other endpoints that we need to create for our todo application.
Expand the file to include the following GET, POST, PUT, and DELETE endpoints.
...
\begin{code}[language=html,numbers=none]{}
  ...
  Server: Werkzeug/2.2.3 Python/3.10.9
  Date: Fri, 20 Feb 2026 04:27:42 GMT
...
Now we can start to create the endpoints that we need for our todo application.
In the \texttt{routes.py} file, add the following code to the bottom of the file.
...
The server should reload, if it does not, you can manually reload it by stopping the process and restarting it.
Now if we run the GET request again we should see the following.

\begin{code}[language=json,numbers=none]{}
  HTTP/1.1 200 OK
  Server: Werkzeug/2.2.3 Python/3.10.9
  Date: Fri, 20 Feb 2026 04:38:44 GMT
  Content-Type: application/json
  Content-Length: 3
  Connection: close
...
This endpoint is for listing all the todos that the user has.
For now we are going to return a hard coded todo item so we can get used to having the API return data.
In the \texttt{routes.py} file, modify the \texttt{get\_todos} function to return a hard coded todo item.
...
\begin{code}[language=json,numbers=none]{}
  HTTP/1.1 200 OK
  Server: Werkzeug/2.2.3 Python/3.10.9
  Date: Fri, 20 Feb 2026 04:44:00 GMT
  Content-Type: application/json
  Content-Length: 200
  Connection: close
...
Next is our endpoint to get an individual todo by its id.
In the \texttt{routes.py} file, add the following code to the bottom of the file.
...
You will notice in this function we have a single parameter which is the ID fetched from the URL.
You can see this in the \texttt{<int:id>} part of the route annotation.
This is a Flask feature that allows you to fetch parameters from the URL. 

These were our read only methods, now let's get to the mutating methods.
First is our endpoint to create a new todo.
In the \texttt{routes.py} file, add the following code to the bottom of the file.
...
You will notice that currently this function is the same as the GET request.
In future weeks we will build out the functionality to actually create a todo.

Next is our endpoint to update a todo.
In the \texttt{routes.py} file, add the following code to the bottom of the file.
...
Likewise with the POST request, this function is the same as the GET request.
In future weeks we will build out the functionality to actually update a todo.

Finally is our endpoint to delete a todo.
In the \texttt{routes.py} file, add the following code to the bottom of the file.
...
Likewise with the POST request, this function is the same as the GET request.
In future weeks we will build out the functionality to actually delete a todo.

With the endpoints you may have noticed us defining the methods,
this is because we want to define which HTTP methods are allowed for each endpoint.
For example, we do not want a user to be able to delete a todo by sending a GET request, we want them to send a DELETE request.
