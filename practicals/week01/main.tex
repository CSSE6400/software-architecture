...
\info{
  ...
  If you are unable to install these tools, you should be able to use GitHub Codespaces to complete the practical.
}
...
\teacher{
  ...
  \begin{itemize}
    ...
    \item PipEnv is installed via apt instead of through pip. This one is hard to tell without having some intution. Look for being able to uninstall pipenv via pip and its still runnable or by doing an apt list-installed. Fix is to remove pipenv via apt and reinstall via pip.
    \item PipEnv can take a significant amount of time on a fresh WSL2 for some reason. Unknown if theres a real fix for this or if its an ongoing issue.
  \end{itemize}
}
...
\warning{Depending on your system you may have to use \texttt{python3 pipenv --python 3} instead of \texttt{pipenv --python 3}}
...
\info{Your Python version may be different, this is fine as long as it is Python 3. For our automated tests we would like you to change the version number from the \texttt{Pipfile} so that it is \texttt{python\_version = "3"}}
