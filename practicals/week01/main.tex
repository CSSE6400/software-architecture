...
\info{
  ...
  If you are running a version of Python lower than this then please consider upgrading.
}
...
Now we can start to create the endpoints that we need for our todo application. In the \texttt{routes.py} file,
...
Now the server should reload, if it does not, you can manually reload it by stopping the process and restarting it.
...
but in future weeks we will build out the functionality to actually create a todo.
...
but in future weeks we will build out the functionality to actually update a todo.
...
Likewise with the POST request, this function is the same as the GET request
but in future weeks we will build out the functionality to actually delete a todo.
