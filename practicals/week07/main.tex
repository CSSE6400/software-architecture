...
\aside{
  ...
  \url{https://edstem.org/au/courses/33564/discussion/3127162}
}
...
We will demonstrate working with the two flavours of queues provided by AWS SQS.
...
After pulling the queue image, it should output the following.
...
\warning{
  There appears to be a bug in GitHub's Container Registry (GHCR) in relation to public images.
  If you receive the following error message from Docker: \newline
  \texttt{docker: Error response from daemon: error from registry: denied}\newline
  It may be because Docker is logged into \texttt{ghcr.io}.
  Try logging out using \texttt{docker logout ghcr.io}, in order to be able to pull the public image.
}
...
Refresh the SQS dashboard to see in flight and available change as the messages are processed.
...
\info{
  When making multiple publishers, you may want to change the \texttt{client-name} CLI parameter,
...
\warning{
  \textbf{Please remember to delete your resources using \texttt{terraform destroy}!}
}
...
\bash{pipx run poetry add celery redis celery\[sqs\] boto3}

\noindent
The above will install the Celery core, Redis, and AWS SQS drivers for communication.
Start the local Redis queue using \texttt{docker compose}.

\bash{docker compose up redis}

\noindent
Create the environment variables Celery needs to access the local Redis queue.
Then start the worker to ensure that the queue and worker can communicate.

\begin{code}[language=bash,numbers=none]{}
$ export CELERY_BROKER_URL=redis://localhost:6379
$ export CELERY_RESULT_BACKEND=redis://localhost:6379
$ pipx run poetry run python3 -m celery --app todo.tasks.ical worker --loglevel=info
\end{code}
% Windows specific:
% $ pipx run poetry run python3 -m celery --app todo.tasks.ical worker --loglevel=info --pool=solo
% Windows doesn't support Celery's default prefork pool.

\noindent
You should see something like the following example output.
...
Modify the \texttt{docker-compose.yml} file as follows.
...
\begin{code}[numbers=none]{docker-compose.yml}
  worker:
    ...
    command: pipx run poetry run celery --app todo.tasks.ical worker --loglevel=info
\end{code}

Note that we are re-using the same development image as the Flask server.
This is for simplicity now, since it shares very similar dependencies.
...
\vspace{-0.3em}
Try sending a POST request to the \texttt{/api/v1/todos/ical} endpoint
to start a job and follow the subsequent URLs, from Section \ref{sect:apiext}, to see the result of the job.
...
\vspace{-0.3em}
\bash{pipx run poetry add icalendar}
...
Update the \texttt{docker-compose.yml} file as below to allow Celery to use our PostgreSQL database.
%
\begin{code}[numbers=none]{docker-compose.yml}
    environment:
