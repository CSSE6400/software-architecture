...
for folder in ${REPO_ROOT}/examples/*; do
    echo $folder
    ...
    if [[ -f "${folder}/latexmkrc" ]]; then
        ...
        if [ $? -ne 0 ]; then
            echo "error: $(basename $folder) examples failed to build"
            exit 1
        fi
        ...
        if [ $? -ne 0 ]; then
            echo "error: $(basename $folder) examples failed to build instructor version"
            exit 1
        fi
