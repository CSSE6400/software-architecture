...
In the \textit{Distributed Systems I} notes \cite{distributed1-notes} and lecture \cite{distributed1-slides},
...
auto-scaling groups and load-balancers can be used to scale-out the service ---
distributing the load to arbitrary copies of the service.
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.75	\textwidth]{diagrams/SaharaScaled}
\end{center}
\vspace{-0.75em}
...
purchasing more powerful machines on which to host the database,
...
In such cases, replication can improve read capacity whilst keeping the write capacity roughly equivalent.
Replication involves creating complete copies of the database, called replicas,
...
so we call this an \textit{eventually consistent} system.
...
For our example, we might naively introduce a system such as Figure \ref{fig:sahara-multi-leader}.
Here we have introduced a second leader, and each leader has their own follower.
This type of separation might make sense.
We might find it beneficial to have a separate database replica in the warehouse,
which can be interacted with via the fulfilment service.
...
Consider a situation where the fulfilment centre notices a defective
\href{https://www.amazon.com.au/dp/B09LYDDDR1}{Nicholas Cage Reversible Pillow}.
They promptly update their system to reflect the decreased stock.
However, at around the same time,
the CSSE6400 tutors place a bulk order for all of Sahara's remaining stock.
Both write operations appear successful to the fulfilment team and the tutors but a conflict has occurred ---
this is known as a \textit{write conflict}, and it is a common problem in multi-leader replications. 
...
\begin{itemize}
    \item Assign IDs to writes, and decide which conflicting write to accept based on the ID
          via some strategy (i.e. highest ID). This results in lost data.
    \item Assign an explicit priority to leader replicas.
          For example, if there is a merge we might accept Leader Replica 1 as the source of truth.
          This also results in lost data.
    \item Merge the values together via some strategy.
          This works for data such as text documents but is inappropriate for stocks of products.
    \item Application code resolution. As most conflict resolution is application dependent,
          many databases allow users to write code which can be executed on write of a conflict or
          read of a conflict to resolve the conflict.
          Where appropriate, our application could even ask the user to resolve the conflict.
\end{itemize}
...
To accomplish this, leaderless databases introduce a few additional constraints on both read and write operations
...
\begin{figure}[ht]
    ...
    \begin{adjustwidth}{-9mm}{-9mm}
        \includegraphics[width=0.965\paperwidth]{diagrams/Leaderless}
    \end{adjustwidth}
...
\begin{figure}[ht]
    \begin{center}
    \includegraphics[width=0.65\textwidth]{diagrams/LeaderlessCoordinator}
    \end{center}
    \vspace{-1em}
...
In leaderless databases, any database node may go offline at any point and the system should continue to function,
assuming only a few nodes go offline.
...
As a simple example, take Figure \ref{fig:sahara-leaderless}, we have three replicas of our database.
...
If instead, we write to two replicas then at most one replica will have outdated data.
Then we need only read from two replicas to ensure that at least one will have the most recent writes.

To generalise, if we have a leaderless database of $n$ replicas, we need to satisfy that,
\vspace{-0.25em}
$$
w + r > n
$$
...
By satisfying this equation we have quorum consistency, the set of writes and reads must overlap,
which means that reading stale data is \textit{unlikely}%
...
hashing the key of a record allows us to maximise the spread of partition usage.
...
Databases often rely on secondary indices to perform a range of useful queries.
...
This load balancer will send requests to the wrong nodes.
...
We considered approaches for coping with undue load on individual nodes via replication.
We looked at approaches for handling greater volume of data than can be stored on a single machine via partitioning.
We noted that often one might want to combine the approaches of replication and partitioning for the one database system.
Both of these techniques introduce a new range of complexity for programmers to deal with.
In our final section we briefly introduced how transactions, and the guarantees they provide,
can help developers in reasoning about their database interactions.

This lecture note is a very brief introduction into the topics of replication, partitioning, and transactions.
