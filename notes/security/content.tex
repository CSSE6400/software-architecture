...
While it seems that developers were locked out of their buildings, the data centres was not affected.
...
This example highlights the key difference between a system which \textsl{fails safely} and a system which \textsl{fails securely}.
...
The principle of Keep it Simple `Silly' (KISS) is needed as complicated software or processes are more often than not, insecure.
...
There are techniques which I've used to create `assignments' which pass all the functionality tests for any given semester.
...
It is also common place to violate the principle slightly by trying to built systems which don't rely on secrecy but keeping the systems secret `just in case'.
...
Violation of this principle has relatively minimal impacts,
this can mean that it is not one which gets considered during software design.
