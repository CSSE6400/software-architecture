...
In sophisticated architectures there are generally two layers of configuration management,
...
In this course, our focus when looking at Infrastructure as Code (IaC) is \textit{stack configuration}.
We rely on containerisation tools, such as Docker, to fill the hole left by the lack of treatment of machine configuration.
...
With virtualisation one physical machine can be the home of numerous virtual machines.
...
There is an assortment of infrastructure code out in the world today,
ranging from simple shell scripts up to more sophisticated tools such as \link{Ansible}{https://docs.ansible.com/},
\link{OpenTofu}{https://opentofu.org/}, and \link{Terraform}{https://developer.hashicorp.com/terraform}.
...
To explore an example of the range of infrastructure code available,
skim the examples below of infrastructure code in BASH, Python, and Terraform.
Each snippet performs the same functionality.
Infrastructure Code does not have to use strictly tools such as Terraform,
...
% Diagram to visualise the extent of automation supported by different languages and tools.
% \begin{figure}[ht]
%     \centering
%     \begin{tikzpicture}[
...
There is however, one significant catch with versioning infrastructure code: \textit{state}.
Most Infrastructure Code tools utilise some form of state.
In Terraform, applying changes to infrastructure updates a file called \textit{terraform.tfstate}.
...
An non-version controlled file such as this might seem like a step backwards, however,
if you consider it as a cache sitting between your Infrastructure Code and your infrastructure,
it is a \textit{slightly} nicer pill to swallow.

\subsection{Automation}
The goal of Infrastructure Code is to help developers manage the complexity of resource management via automation.
In order to maximise our confidence that Infrastructure Code is reflected in reality,
it is important that our build pipelines provide an assurance of consistency.

There are two methods for achieving consistency through automation:
\begin{itemize}[topsep=5pt,partopsep=2pt,itemsep=3pt,parsep=4pt]
    \item fail the pipeline if the Infrastructure Code would change the reality; or
    \item make changes to reality in our pipeline.
\end{itemize}
...
it is the core practice of our industry.
When developing Infrastructure Code it is important to remember that you are (likely) not the first to solve this problem.
You are not the first person to connect a public EC2 instance to an S3 bucket.
A good coding practice that happens to be a win-win for everyone is code reuse.
You cannot copy and paste another developer's mouse clicks to configure their server via a UI,
...
\subsection{Testing}
We will cover testing in the lecture :)
...
We termed this practice Infrastructure as Code, despite the tools (Infrastructure Code) and practices (Infrastructure as Code)
being combined in the real world as Infrastructure as Code.
It is important to note that IaC is still in relatively early stages of development.
There are a number of issues which need to be addressed, such as importing existing infrastructure,
improving state management, and enabling proper refactoring.
Despite these issues, it is the best system we have to date, and a significant improvement on manual management.
