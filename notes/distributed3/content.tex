...
\begin{figure}[h!]
    \centering
    \includegraphics[trim=55 152 358 17,clip,width=0.7\textwidth]{diagrams/eventual-consistency-seq.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[trim=60 189 450 35,clip,width=0.68\textwidth]{diagrams/lamport-timestamp-seq.png}
...
If two orders were sent to \texttt{ProductDB1} and \texttt{ProductDB2} at the same time and there was only one item in stock,
Lamport timestamps would provide a way to decide which order occurred first, but only after the orders were processed.
