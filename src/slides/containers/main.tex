...
A way of \highlight{packaging software}, and its \highlight{dependencies},
such that the software can be run in \highlight{numerous environments}.
}
...
/usr/bin/env: 'python3': No such file or directory
    ...
    Need to install Python and all it's dependencies
\end{frame}
...
SyntaxError: invalid syntax
    ...
    \pause
    \large
    \texttt{f-strings} aren't supported in Python 3.5
...
\end{frame}
\note[itemize]{
    \item Python / numpy versions give different output
    \item Took a fair bit of effort to find this obscure difference
}
...
\end{frame}
\note[itemize]{
    \item First chroot -- \texttt{ls} doesn't exist from our new root
    \item Second chroot -- \texttt{ls} exists but it can't find its \highlight{dependencies}
}
...
\end{frame}
\note[itemize]{
    \item \texttt{ldd} -- List \texttt{ls} dependencies
    \item \texttt{cp} -- Copy these dependencies
    \item \texttt{ls} -- Show that these are now within our directory
}
...
\end{frame}
\note[itemize]{
    \item Can now run \texttt{chroot . /bin/ls} -- Lists everything available from new root
    ...
    \item Fourth chroot -- Shows contents of \texttt{/bin} directory
}
...
\note[itemize]{
    \item Projection -- view layers as if merged together
    \item Copy -- Create copy in an upper layer, if you write to an existing file
    \item Lower Layers -- Read only
}
...
\note[itemize]{
    ...
    \item Lower layer is \highlight{read} only
}
...
\begin{frame}[fragile]{Exploring a \highlight{layered filesystem} \textit{(cont.)}}
\begin{code}[numbers=none]{}
...
\end{frame}
\note{
    First to utilize namespaces, didn't require Linux patches
    \begin{itemize}
        \item Primarily due to use of Process ID namespace
        \item Built on top of earlier namespaces
    \end{itemize}
}
...
\note[itemize]{
    \item The mount point for starting containers
    \item In Docker and rkt the image is just a collection of layers
    \item i.e. lower, and upper, repeatedly stacked on top of themselves
    \item In LXC it is just one layer
}
...
\end{frame}
\note[itemize]{
    \item \texttt{docker run} -- Execute a container based on an image
    \item \texttt{ubuntu} -- Image defining the container
    \item \texttt{/bin/bash} -- What to run in our container process
    \item \texttt{-it} -- Interactive mode
    \vspace{1em}
    \item Second \texttt{docker run} just runs \texttt{ls} and exits when \texttt{ls} finishes
}
...
\point[\huge Creating a Container Image]{
	...
	\note{
	    Layers allow us to have a single copy of items in a layer
	    \begin{itemize}
	        \item e.g. ubuntu -- Single copy on our system that can be used by many other containers
	    \end{itemize}
	}
...
\end{code}
\note{
	Package indices are deleted from ubuntu image to save space\\
	Need \texttt{apt-get update} before you can install anything\\
}
...
\note{
	\vspace{3em}
	\texttt{-t} tags image with a name ``cowsay''\\
	\texttt{.} search current directory for \texttt{Dockerfile}
}
...
CMD ["/usr/games/cowsay", "Hello World"]
    ...
    \end{columns}
\end{frame}
\note{\texttt{CMD} is not a layer, it executes a command on the layers.}
...
\end{frame}
\note{
	Add command to remove package indices to save space\\~\\
	Question: Is there a problem with this logic?
	\begin{itemize}
		\item Layered file system -- Layers are read only
		\item Does not remove items from lower layer
	\end{itemize}
}
...
\end{frame}
\note{
    Retrieve package indices, install features, remove indices, all in one layer
...
\end{columns}
\end{frame}
\note{In Docker example, if the File Systems are the same, they are not duplicated}
...
\note{Virtual machine when you need to fence off the \textit{machine}, not just the process}


\begin{frame}{Size Comparison}

    I want \highlight{10} Flask servers running on Ubuntu 26

    \normalsize
    \begin{align*}
        \text{Ubuntu 26} &\simeq 5.9GB \\
        \text{Python 3.13.12} &\simeq 649MB \\
...
\note{OS Example: Windows on Linux}
...
\note{Quick startup of containers vs VMs benefits scaling}
...
\note{British slang for dock worker}
...
\end{frame}
\note[itemize]{
	\item Build front and backends
	\item Run front and backends with open ports to access them
	\item Run reverse proxy to allow access to both via port 80
}
