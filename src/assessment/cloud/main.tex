...
For this assignment, you are working for AgeOverflow, a new UQ start-up.
AgeOverflow uses (fictional) machine learning techniques.
...
\paragraph{Task}
AgeOverflow uses a microservices architecture to implement their analysis platform.
...
Manual verification by personnel is labour intensive and time consuming.
Automated analysis provides faster responses with varying degrees of certainty, to allow manual review only when its required.
...
\textbf{Version 1.0.1}: Ordering of lists returned from \texttt{GET} queries is specified.

\textbf{Version 1.0.2}: Corrected specification of ages in the 80 bucket for returned statistics.

\textbf{Version 1.0.3}: Corrected 404 error conditions for \texttt{GET /analysis/{customer\_id}/requests/{request\_id}}.
...
You must use the provided command line tool (\texttt{ageoverflow-engine}) to analyse the image sets sent to your service.
...
\noindent
Do \textbf{\textit{not}} include this binary in your assignment repository.
The README documentation in the tool's repository describes how to copy it into your assignment's Docker image.
...
The most up-to-date list of available services, and restrictions on their usage,
is described in the Learner Lab \texttt{Readme} tab.
...
\subsection{Scaling}
A simplistic approach to manage high volumes of requests or high processing demands,
is to have many compute nodes running to cater for the demand.
Stage 3 of this assignment assesses your ability to scale your application to meet demand.
This means your application needs to scale up \textit{\textbf{and}} down to be economically efficient.
From an evaluation perspective, this means
\begin{itemize}[itemsep=3pt,parsep=3pt,topsep=3pt]
	\item starting your application with many compute nodes available and idle,
	\item scaling up faster than demand requires,
	\item or not scaling down in a timely manner,
\end{itemize}
will limit your final grade for stage 3. See the criteria on page \pageref{criteria} for details.

\subsection{AWS Budget}
You have been provisioned two Learner Labs for this course.
One is for practical exercises and experimentation.
The other is for this assignment.
Each lab has a budget of \textbf{\$50 USD} to spend on AWS resource usage.
You will no longer be able to access your lab if you exceed this budget.
You need to ensure you do not leave services running when you do not need them.
Some services can consume your entire budget in just a few days.
You also need to ensure you do not go overboard in stage 3,
by scaling up so many services that they consume a large portion of your budget during testing.
...
\begin{enumerate}[topsep=5pt,partopsep=2pt,itemsep=3pt,parsep=4pt]
...
\begin{itemize}[topsep=6pt,partopsep=2pt,itemsep=3pt,parsep=4pt]
...
\begin{itemize}
  ...
  \item A \texttt{local.sh} script that can be used to build and run your service locally.
        ...
        to the testing environment, and your service \textbf{must} be available at \texttt{http://localhost:8080/api/v1/}.
\end{itemize}
...
\begin{enumerate}
  ...
  \item If a \texttt{before.sh} file exists in the root directory, it will be run.
  \item We will run \texttt{terraform init}, \texttt{terraform plan}, and \texttt{terraform apply -auto-approve}
        in the root directory of your project.
  \item We will then run \texttt{terraform output -raw api > api.txt}.
        The result of this must be that \texttt{api.txt} contains the URL at which your API is deployed
        (e.g. \texttt{http://my-api.com/api/v1/} or \\
        \texttt{http://123.231.213.012/api/v1/}).
        The \texttt{api.txt} file must \textbf{only} contain the URL.
        Do not include \textbf{\textit{any}} other text in the output.
  \item Automated functionality and load tests will be run against the URL discovered in the \texttt{api.txt} file.
\end{enumerate}
...
A subset of the functionality tests will be released before the first submission deadline.
